...
function AppendScore(quizName, score, maxScore, timeTaken){
...
function ForEachPastScore(filter, callback, emptyScoreCallback){
    ...
    for(let i = scores.length-1; i >= 0; i--){
        ...
        let time = scoreData.time;

        if(filter == null || scoreData.name == filter)
            callback(scoreData.name, score, maxScore, time)
    }
}
